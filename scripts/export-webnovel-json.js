#!/usr/bin/env node
/**
 * Export canonical data/webnovel.json from embedded WebNovel TOC seeds.
 * Run: node scripts/export-webnovel-json.js [--out path]
 */
const fs = require('fs');
const path = require('path');
const { SCHEMA, DEFAULT_JSON } = require('./lib/webnovel-json');

const ROOT = path.join(__dirname, '..');

/** WebNovel SVSSS ID translation — Volume 1 · 109 chapters */
...
function seedSvsssCatalog() {
    return SVSSS_CHAPTERS.map(([chapterId, title, isExtra], i) => ({
        n: i + 1,
        chapterId,
        title,
...
function seedSoloCatalog() {
    return SOLO_CHAPTERS.map(([chapterId, title, index]) => ({
        chapterId,
        title,
        index
    }));
...
function buildExportDocument() {
    return {
        ...
        source: {
            ...
            soloLeveling: 'https://www.webnovel.com/comic/15227640605485101'
        },
        packages: {
            svsss: {
                id: 'svsss',
        ...
        catalogs: {
            svsss: seedSvsssCatalog(),
            soloLeveling: seedSoloCatalog()
        },
        integratedChapters: {
            svsss: [
                {
...
const outPath = outArg >= 0 ? process.argv[outArg + 1] : DEFAULT_JSON;
...
console.log(`  SVSSS catalog: ${doc.catalogs.svsss.length} chapters`);
console.log(`  Solo catalog: ${doc.catalogs.soloLeveling.length} chapters`);
console.log(`  Integrated SVSSS: ${doc.integratedChapters.svsss.length}`);
console.log(`  Integrated Solo: ${doc.integratedChapters.soloLeveling.length}`);